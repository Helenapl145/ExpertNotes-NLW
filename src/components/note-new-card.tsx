...
export function NewNoteCard({onNoteCreated}: NewNoteCardProps){
    ...
    function handleStartRecording(){
        const isSperchRecognitionAPIAvailabe = 'SpeechRecognition' in window
            || 'webkitSpeechRecognition' in window

    
        if(!isSperchRecognitionAPIAvailabe) {
    ...
    return (
        <Dialog.Root>
            <Dialog.Trigger className='rounded-md bg-slate-700 p-5 space-y-3 text-left overflow-hidden outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-600'>
                <span className='text-sm font-radius text-slate-200'>
                Adicionar nota
                </span>
                ...
                <Dialog.Content className='fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none'>
                        ...
                        <form className='flex-1 flex flex-col'>
                            ...
                            <div className='flex flex-1 flex-col gap-3 p-5'>
                                ...
                                <span className='text-sm font-radius text-slate-300'>
                                    Adicionar nota
                                </span>
                            ...
                            {isRecording ? (
                                ...
                                className='w-full bg-lime-400 py-4 text-center text-sm text-lime-950 outline-none font-medium hover:bg-lime-900'
                                > 
                                    Salvar nota
                                </button>
                            )
